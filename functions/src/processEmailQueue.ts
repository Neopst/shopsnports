  ...
  .onCreate(async (snapshot, context) => {
    ...
    try {
      ...
      if (type === 'admin_welcome') {
        const template = await getTemplate('admin_welcome', admin.firestore());
        const { subject: tmplSubject, htmlBody: tmplHtml, plainTextBody: tmplText } = renderTemplate(template, {
        ...
        const template = await getTemplate('password_reset', admin.firestore());
        const { subject: tmplSubject, htmlBody: tmplHtml, plainTextBody: tmplText } = renderTemplate(template, {
