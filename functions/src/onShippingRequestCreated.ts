...
import { validateShippingRequest, ValidationError } from './validation';
 ...
 */
export const onShippingRequestCreated = async (
  ...
  try {
    ...
    try {
      ...
      if (validationError instanceof ValidationError) {
        ...
        throw new functions.https.HttpsError(
          ...
          { field: validationError.field, code: validationError.code }
        );
      }
    ...
    let affiliateTokenId: string | null = null;
    if (requestData.affiliateToken) {
      try {
        ...
        if (!tokenQuery.empty) {
          ...
          if (tokenData.used) {
            console.warn(
              `⚠️ Token ${requestData.affiliateToken} already used for request: ${tokenData.usedForRequest}`
            );
          } else if (new Date() > tokenData.expiresAt.toDate()) {
            console.warn(
              `⚠️ Token ${requestData.affiliateToken} has expired`
            );
          } else {
            // Mark token as used
            await markTokenUsed(
              db,
              requestData.affiliateToken,
              requestId,
            ...
            if (tokenData.affiliateId) {
              ...
              console.log(
                `✅ Affiliate ${tokenData.affiliateId} auto-tagged from token`
              );
            }
          }
        } else {
    ...
    const adminNotification = {
      ...
      message: `New shipping request from ${updatedRequestData.clientName || updatedRequestData.senderName}. Destination: ${updatedRequestData.destination || updatedRequestData.destinationLocation}${
    ...
    if (updatedRequestData.affiliate) {
      const affiliateNotification = {
        ...
        read: false,
        createdAt: admin.firestore.FieldValue.serverTimestamp(),
        message: `New referral: ${updatedRequestData.senderName} submitted a shipping request (${updatedRequestData.destinationLocation})${
    ...
    if (adminTokens.length > 0) {
      try {
        ...
        console.log(
          `Sent FCM to ${adminFcmResponse.successCount} admin device(s)`
        );
      ...
      try {
        ...
        if (affiliateQuery.exists) {
          ...
          if (affiliateData?.fcmTokens && Array.isArray(affiliateData.fcmTokens)) {
            ...
            console.log(
              `Sent FCM to affiliate: ${affiliateFcmResponse.successCount} device(s)`
            );
          }
        }
