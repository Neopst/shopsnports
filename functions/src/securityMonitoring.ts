...
export enum SecurityEventType {
  ...
  RAPID_PASSWORD_CHANGES = 'rapid_password_changes',
 ...
 */
export const trackFailedLogin = functions.https.onCall(
  async (data, context) => {
    try {
      ...
      if (!email || !ipAddress) {
        throw new functions.https.HttpsError(
          'invalid-argument',
          'Email and IP address are required'
        );
      }
 ...
 */
export const monitorAdminActivity = functions.firestore
 ...
 */
async function createSecurityAlert(event: SecurityEvent): Promise<void> {
  const db = admin.firestore();
 ...
 */
export const getSecurityAlerts = functions.https.onCall(
  async (data, context) => {
    try {
      if (!context.auth) {
        throw new functions.https.HttpsError(
 ...
 */
export const acknowledgeSecurityAlert = functions.https.onCall(
  async (data, context) => {
    try {
      if (!context.auth) {
        throw new functions.https.HttpsError(
      ...
      if (!callerDoc.exists || callerDoc.data()!.role !== 'super_admin') {
        throw new functions.https.HttpsError(
          'permission-denied',
          'Only super admins can acknowledge security alerts'
        );
      }
 ...
 */
export const clearFailedLoginAttempts = functions.https.onCall(
  async (data, context) => {
    try {
      if (!context.auth) {
        throw new functions.https.HttpsError(
      ...
      if (!callerDoc.exists || callerDoc.data()!.role !== 'super_admin') {
        throw new functions.https.HttpsError(
          'permission-denied',
          'Only super admins can clear failed login attempts'
        );
      }
 ...
 */
export const unlockAdminAccount = functions.https.onCall(
  async (data, context) => {
    try {
      if (!context.auth) {
        throw new functions.https.HttpsError(
      ...
      if (!callerDoc.exists || callerDoc.data()!.role !== 'super_admin') {
        throw new functions.https.HttpsError(
          'permission-denied',
          'Only super admins can unlock admin accounts'
        );
      }
