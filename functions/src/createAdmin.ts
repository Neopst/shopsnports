...
import { validateAdminCreation, ValidationError } from './validation';
...
function generateSecurePassword(): string {
  ...
  const random = (str: string, n: number) => {
...
export const createAdmin = functions.https.onRequest(async (req, res) => {
  ...
  try {
    ...
    try {
      validateAdminCreation({ email, displayName, permissions });
    } catch (error) {
 ...
 */
export const resetAdminPassword = functions.https.onRequest(async (req, res) => {
  const corsConfig = getCorsConfig();
 ...
 */
function _formatPermissionsList(permissions: Record<string, boolean>): string {
