 ...
 */
export const logAdminActivity = functions.https.onCall(
  async (data, context) => {
    try {
      if (!context.auth) {
      ...
      const {
        action, // Enum: login, logout, created_admin, updated_permissions, etc.
 ...
 */
export const onAdminLogin = functions.https.onCall(
  async (data, context) => {
    try {
      if (!context.auth) {
        throw new functions.https.HttpsError(
 ...
 */
export const onAdminLogout = functions.https.onCall(
  async (data, context) => {
    try {
      if (!context.auth) {
        throw new functions.https.HttpsError(
 ...
 */
export const getAdminActivityStats = functions.https.onCall(
  async (data, context) => {
    try {
      if (!context.auth) {
        throw new functions.https.HttpsError(
      ...
      if (!isSuperAdmin && callerId !== targetAdminId) {
        throw new functions.https.HttpsError(
          'permission-denied',
          'You can only view your own activity stats'
        );
      }
      ...
      logsQuery.docs.forEach((doc) => {
        ...
        else if (action === 'logout') logoutCount++;
        else {
      ...
      console.log(
        `📊 Activity stats retrieved for admin: ${targetAdminId}, ${logsQuery.size} total activities`
      );
 ...
 */
export const getAdminActivityLogs = functions.https.onCall(
  async (data, context) => {
    try {
      if (!context.auth) {
        throw new functions.https.HttpsError(
      ...
      console.log(
        `📋 Retrieved ${logs.length} activity logs for admin: ${targetAdminId}`
      );
