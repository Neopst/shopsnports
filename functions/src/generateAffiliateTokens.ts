 ...
 */
function generateTokenSequence(): string {
 ...
 */
export const generateAffiliateTokens = functions.https.onCall(
  async (data, context) => {
    try {
      ...
      try {
        validateNumber(batchSize, { min: 1, max: 50, fieldName: 'batchSize' });
      ...
      console.log(
        `🎫 Generating ${batchSize} tokens for affiliate: ${affiliateData.fullName}`
      );
 ...
 */
export const adminGenerateAffiliateTokens = functions.https.onCall(
  async (data, context) => {
    try {
      ...
      try {
        ...
        validateNumber(batchSize, { min: 1, max: 100, fieldName: 'batchSize' });
      } catch (validationError) {
        if (validationError instanceof ValidationError) {
          throw new functions.https.HttpsError('invalid-argument', validationError.message);
        }
      ...
      if (!affiliateDoc.exists) {
        throw new functions.https.HttpsError(
          'not-found',
          `Affiliate ${affiliateId} not found`
        );
      }
      ...
      console.log(
        `👤 Admin generating ${batchSize} tokens for affiliate: ${affiliateData.fullName}`
      );
      ...
      console.log(
        `✅ Admin generated ${batchSize} tokens for affiliate ${affiliateId}`
      );
 ...
 */
export const validateAffiliateToken = functions.https.onCall(
  async (data, context) => {
    try {
      const { token } = data;
      ...
      if (tokenQuery.empty) {
        throw new functions.https.HttpsError(
          'not-found',
          'Token not found. Please check the token and try again.'
        );
      }
      ...
      if (tokenData.used) {
        throw new functions.https.HttpsError(
          'already-exists',
          `Token already used for request: ${tokenData.usedForRequest}`
        );
      }
