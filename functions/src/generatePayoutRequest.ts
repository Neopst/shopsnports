...
import { validateString, validateNumber, ValidationError } from './validation';
import { calculateTax } from './taxCalculation';
import { createPaymentService, PayoutData } from './paymentService';
 ...
 */
function validatePayoutRequestInput(data: any): void {
  ...
  if (!commissionIds || !Array.isArray(commissionIds)) {
    throw new ValidationError('Commission IDs must be an array', 'commissionIds', 'INVALID_TYPE');
  }

  if (commissionIds.length === 0) {
    throw new ValidationError('At least one commission must be selected', 'commissionIds', 'EMPTY');
  }

  if (commissionIds.length > 100) {
    throw new ValidationError('Cannot include more than 100 commissions in a single payout', 'commissionIds', 'TOO_MANY');
  }
...
export const generatePayoutRequest = async (
  data: {
    affiliateId: string;
  ...
  try {
    ...
    if (!context.auth.token.admin) {
      throw new functions.https.HttpsError(
        'permission-denied',
        'Only admins can generate payout requests'
      );
    }
    ...
    const commissionSnapshots = await Promise.all(
      commissionIds.map((id) => db.collection('commissions').doc(id).get())
    );
    ...
    for (const snap of commissionSnapshots) {
      ...
      if (commission.affiliateId !== affiliateId) {
        throw new functions.https.HttpsError(
          'permission-denied',
          `Commission ${snap.id} does not belong to affiliate ${affiliateId}`
        );
      }
      ...
      if (commission.status === 'paid') {
        throw new functions.https.HttpsError(
          'invalid-argument',
          `Commission ${snap.id} is already paid`
        );
      }
    ...
    if (totalAmount <= 0) {
      throw new functions.https.HttpsError(
        'invalid-argument',
        'Total payout amount must be greater than 0'
      );
    }
...
export const processPayout = async (
  data: {
    payoutId: string;
    transactionReference?: string; // Bank transfer ID, check number, etc
    notes?: string;
    paymentProvider?: string; // 'manual', 'stripe', 'paypal'
  },
  context: functions.https.CallableContext
) => {
  try {
    ...
    const payoutRequestData: PayoutData = {
      payoutId,
...
export const bulkProcessPayouts = async (
  ...
  try {
    ...
    if (!payoutIds || !Array.isArray(payoutIds) || payoutIds.length === 0) {
      throw new functions.https.HttpsError(
        'invalid-argument',
        'At least one payout ID must be provided'
      );
    }
    ...
    if (payoutIds.length > 50) {
      throw new functions.https.HttpsError(
        'invalid-argument',
        'Cannot process more than 50 payouts at once'
      );
    }
    ...
    const payoutDocs = await Promise.all(
      payoutIds.map((id) => db.collection('payouts').doc(id).get())
    );
...
export const exportPayouts = async (
  ...
  try {
    if (!context.auth?.token.admin) {
      throw new functions.https.HttpsError(
        'permission-denied',
        'Only admins can export payouts'
      );
    }
