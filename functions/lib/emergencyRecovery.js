...
exports.cancelRecoveryRequest = exports.getPendingRecoveryRequests = exports.completeEmergencyRecovery = exports.requestEmergencyRecovery = void 0;
 ...
 */
exports.requestEmergencyRecovery = functions.https.onRequest(async (req, res) => {
    try {
 ...
 * and restore access.
 */
exports.completeEmergencyRecovery = functions.https.onRequest(async (req, res) => {
    try {
 ...
 */
exports.getPendingRecoveryRequests = functions.https.onCall(async (data, context) => {
    try {
        if (!context.auth) {
        ...
        if (!callerDoc.exists || callerDoc.data().role !== 'super_admin') {
            throw new functions.https.HttpsError('permission-denied', 'Only super admins can view recovery requests');
        }
 ...
 */
exports.cancelRecoveryRequest = functions.https.onCall(async (data, context) => {
    try {
        if (!context.auth) {
            throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
        }
        ...
        if (!recoveryId) {
            throw new functions.https.HttpsError('invalid-argument', 'Recovery ID is required');
        }
        const db = admin.firestore();
        // Verify caller is super admin
        const callerDoc = await db.collection('admin_users').doc(callerId).get();
        if (!callerDoc.exists || callerDoc.data().role !== 'super_admin') {
            throw new functions.https.HttpsError('permission-denied', 'Only super admins can cancel recovery requests');
        }
