...
exports.onShipmentRequestCreated = onShipmentRequestCreated;
const admin = __importStar(require("firebase-admin"));
// Removed admin.initializeApp() - Firebase Functions auto-initializes firebase-admin
async function onShipmentRequestCreated(snapshot, context) {
    ...
    try {
        ...
        await admin.messaging().sendToTopic('admins', payloadForAdmin);
        if (affiliateId) {
            ...
            await admin.messaging().sendToTopic(topic, payloadForAff);
        }
    }
    catch (err) {
