 ...
 */
async function getTemplate(type, db) {
    try {
 ...
 */
async function sendTemplatedEmail(db, transporter, context, templateType) {
    try {
        const template = await getTemplate(templateType, db);
        const { subject, htmlBody, plainTextBody } = renderTemplate(template, context.variables || {});
        await transporter.sendMail({
            from: (0, smtpConfig_1.getSmtpConfig)().user,
            to: context.to,
