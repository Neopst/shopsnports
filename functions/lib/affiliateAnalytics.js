...
exports.getAffiliateLeaderboard = exports.getAffiliateAnalytics = void 0;
 ...
 * - Performance metrics
 */
exports.getAffiliateAnalytics = functions.https.onCall(async (data, context) => {
    try {
        if (!context.auth) {
        ...
        for (const doc of commissionsSnapshot.docs) {
            ...
            const createdAt = commission.createdAt.toDate();
        ...
        for (const doc of formSharesSnapshot.docs) {
            ...
            else if (new Date() > share.expiresAt.toDate()) {
                expiredShares++;
            }
        }
        ...
        return {
            ...
            profile: {
                ...
                joinedDate: affiliateData?.joinedDate?.toDate()?.toISOString(),
            },
        };
    }
    catch (error) {
 ...
 */
exports.getAffiliateLeaderboard = functions.https.onCall(async (data, context) => {
    try {
        ...
        switch (metric) {
            case 'earnings':
                ...
                for (const doc of affiliatesSnapshot.docs) {
                    ...
                    for (const commissionDoc of commissionsSnapshot.docs) {
                        totalEarnings += commissionDoc.data().commissionAmount;
                        totalShipments++;
                    }
                    leaderboard.push({
                        affiliateId,
            ...
            default:
                throw new functions.https.HttpsError('invalid-argument', `Invalid metric: ${metric}`);
        }
