...
exports.getFormShareAnalytics = exports.cleanupExpiredFormShares = exports.generateFormShareLink = exports.onFormShareTokenUsed = void 0;
 ...
 */
exports.onFormShareTokenUsed = functions.https.onCall(async (data, context) => {
    try {
        ...
        try {
            ...
            (0, validation_1.validateString)(formShareToken, { required: true, minLength: 10, maxLength: 200, fieldName: 'formShareToken' });
        }
        ...
        console.log(`📋 Processing form share submission with token: ${formShareToken}`);
        ...
        const tokenQuery = await db
            ...
            .get();
        if (tokenQuery.empty) {
            throw new functions.https.HttpsError('not-found', 'Form share token not found. Link may have expired.');
        }
        const tokenDoc = tokenQuery.docs[0];
        const tokenData = tokenDoc.data();
        // Check if already used
        if (tokenData.used) {
            throw new functions.https.HttpsError('already-exists', `This form link has already been used for request: ${tokenData.resultingShippingRequestId}`);
        }
        // Check if expired (7 days)
        const expiryDate = tokenData.expiresAt.toDate();
        if (new Date() > expiryDate) {
            throw new functions.https.HttpsError('deadline-exceeded', 'Form share link has expired. Ask the affiliate for a new link.');
        }
        ...
        console.log(`✅ Shipping request tagged with affiliate: ${tokenData.affiliateId}`);
        ...
        console.log(`✅ Form share workflow completed for request ${shippingRequestId}`);
        return {
            success: true,
            shippingRequestId,
            affiliateId: tokenData.affiliateId,
 ...
 */
exports.generateFormShareLink = functions.https.onCall(async (data, context) => {
    try {
        if (!context.auth) {
        ...
        console.log(`🔗 Generating form share link for affiliate: ${affiliateData.fullName}`);
        // Generate unique token for this share
        const formShareToken = `SHARE-AFF-${new Date().getFullYear()}-${Math.floor(Math.random() * 100000)
            .toString()
            .padStart(5, '0')}`;
        ...
        console.log(`✅ Form share link created: ${formShareToken} for client: ${clientEmail}`);
 ...
 */
exports.cleanupExpiredFormShares = functions.pubsub
 ...
 */
exports.getFormShareAnalytics = functions.https.onCall(async (data, context) => {
    try {
        if (!context.auth) {
            throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
        }
        ...
        const conversionRate = totalShares > 0 ? ((successfulUses / totalShares) * 100).toFixed(1) : '0';
