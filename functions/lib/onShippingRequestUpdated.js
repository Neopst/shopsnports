...
exports.onShippingRequestUpdated = void 0;
 ...
 */
const onShippingRequestUpdated = async (change, context) => {
    try {
        ...
        const assignedChanged = previousData.assignedTo !== newData.assignedTo;
        ...
        if (statusChanged && newData.status === 'delivered' && newData.affiliate) {
            ...
            if (!existingCommission.empty) {
                console.warn(`⚠️ Commission already exists for shipping request ${requestId} and affiliate ${newData.affiliate}. Skipping duplicate payout generation.`);
            }
            else {
                // Get shipment price (admin must have set this before marking delivered)
                const shipmentPrice = newData.shipmentPrice || 0;
                if (shipmentPrice <= 0) {
                    console.warn(`⚠️ Shipment price not set for request ${requestId}, skipping auto payout generation`);
                }
                else {
                    try {
                        ...
                        if (affiliateDoc.exists) {
                            ...
                            console.log(`✅ Commission created: ${commissionRef.id} for $${commissionAmount.toFixed(2)}`);
                            ...
                            const taxCalculation = await (0, taxCalculation_1.calculateTax)(db, commissionAmount, 'affiliate');
                            ...
                            console.log(`✅ Payout auto-generated: ${payoutRef.id} for $${commissionAmount.toFixed(2)}`);
                            ...
                            try {
                                ...
                                console.log(`✅ Affiliate commission invoice created: ${invoiceRef.id}`);
                                ...
                                console.error(`❌ Error auto-generating affiliate invoice for request ${requestId}:`, invoiceError);
                        ...
                        console.error(`❌ Error auto-generating payout for request ${requestId}:`, payoutError);
        ...
        if (statusChanged) {
            ...
            const statusMessage = statusMessages[newData.status] || 'Status updated';
            ...
            try {
                ...
                if (validation.valid && newData.senderEmail) {
                    ...
                    const template = await (0, emailTemplateService_1.getTemplate)('shipping_status_update', db);
...
exports.onShippingRequestUpdated = onShippingRequestUpdated;
